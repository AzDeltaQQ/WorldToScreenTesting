...
DWORD WINAPI DelayedInitialization(LPVOID lpParam) {
    ...
    try {
        ...
        LOG_INFO("MovementController initialized.");
        ...
        LOG_ERROR("Exception in DelayedInitialization: " + std::string(e.what()));
    }
    catch (...) {
        LOG_ERROR("Unknown exception in DelayedInitialization");
    }
...
static BOOL SafeInitializeLogger(HMODULE hModule) {
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    switch (ul_reason_for_call) {
        case DLL_PROCESS_ATTACH:
        {
            ...
            if (!SafeInitializeLogger(hModule)) {
                return FALSE;
            }
